...
debugmode=0
dumpdir=dump_mfcc40_pitch   # directory to dump full features
...
# exp tag
tag="b" # tag for managing experiments.
...
if [ ${stage} -le 1 ]; then
    ...
    mkdir -p data/mfcc40_pitch
    if false; then
    for x in $train_set $train_dev ${recog_set}; do
        utils/copy_data_dir.sh data/${x} data/mfcc40_pitch/${x}
        steps/make_mfcc_pitch.sh --cmd "$train_cmd" --nj 32 \
          --mfcc-config conf/mfcc_hires.conf \
          --write-utt2num-frames true data/mfcc40_pitch/${x}
        utils/fix_data_dir.sh data/mfcc40_pitch/${x}
    done
    fi
    # compute global CMVN
    compute-cmvn-stats scp:data/mfcc40_pitch/${train_set}/feats.scp data/mfcc40_pitch/${train_set}/cmvn.ark
    dump.sh --cmd "$train_cmd" --nj 32 --do_delta $do_delta \
        data/mfcc40_pitch/${train_set}/feats.scp data/mfcc40_pitch/${train_set}/cmvn.ark exp/dump_feats_mfcc40/train ${feat_tr_dir}
    dump.sh --cmd "$train_cmd" --nj 10 --do_delta $do_delta \
        data/mfcc40_pitch/${train_dev}/feats.scp data/mfcc40_pitch/${train_set}/cmvn.ark exp/dump_feats_mfcc40/dev ${feat_dt_dir}
    for rtask in ${recog_set}; do
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
        mkdir -p ${feat_recog_dir}
        dump.sh --cmd "$train_cmd" --nj 10 --do_delta $do_delta \
            data/mfcc40_pitch/${rtask}/feats.scp data/mfcc40_pitch/${train_set}/cmvn.ark exp/dump_feats_mfcc40/recog/${rtask} \
            ${feat_recog_dir}
    done
fi
